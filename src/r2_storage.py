...
def get_r2_client():
    ...
    secret_key = st.secrets.get("R2_SECRET_ACCESS_KEY") or st.secrets.get("R2_SECRET_KEY")
...
def is_local_dev() -> bool:
    ...
    local_parquet = Path("/Users/maximiliansperlich/Developer/projects/data/brixels_parquet/index.json")
...
def get_local_parquet_dir() -> Path | None:
    ...
    local_dir = Path("/Users/maximiliansperlich/Developer/projects/data/brixels_parquet")
    if local_dir.exists():
        return local_dir
    return None
...
def get_tiles_for_bounds(bounds: tuple, grid_size: str) -> list[dict]:
    ...
    for tile in index["tiles"]:
        ...
        # Check intersection
        if (tile["max_x"] > lon_min and tile["min_x"] < lon_max and
            tile["max_y"] > lat_min and tile["min_y"] < lat_max):
...
def load_grid_data_r2(layer_name: str, bounds: tuple, columns: list) -> gpd.GeoDataFrame:
    """
    ...
    minx, miny, maxx, maxy = bounds
    mask = (
        (result.geometry.x >= minx) & (result.geometry.x <= maxx) &
        (result.geometry.y >= miny) & (result.geometry.y <= maxy)
    )
    result = result[mask]
